...
# Build lookup table: key = first field, value = full line
$lookup = @{}
foreach ($aLine in $aLines) {
    $key = ($aLine -split ':', 2)[0].Trim()
    if ($key -ne '') {
        $lookup[$key] = $aLine
    }
}
...
while (-not $sr.EndOfStream) {
    ...
    if ($parts.Length -ge 4) {
        ...
        if ($lookup.ContainsKey($key)) {
            $aLine = $lookup[$key]
            # Write concatenated B + A on one line
            $writer.WriteLine("$bLine$aLine")
            $totalMatches++
        }
    }
}
